...
Because Navigation menus are stored in the database, the Menu ID will be different on your local test site and your live server. You must do this for each environment.

1. In the target environment: **Appearance → Editor → Navigation → Create new navigation**
2. Build this structure:

| Item | URL | Notes |
|------|-----|-------|
| Blog | `/blog/` | |
| Series | `/series/` | Parent item with dropdown |
| → SOE Fiction Series | `/soe-fiction-series/` | Submenu item |
| → The Adversaries Series | `/adversaries-series/` | Submenu item |
| → The Murray Books | `/murray-books/` | Submenu item |
| Dennis Drayton | `/dennis-drayton/` | |
| Books | `/books/` | |
| Christopher Nugent | `/christopher-nugent/` | |
| Even More | `/even-more/` | |

3. After saving, note the navigation post ID from the URL (e.g. `?post=123`).
4. **Link the ID:** 
   - Open `parts/navigation.html` in the **target environment's theme folder**.
   - Change the `"ref"` value to match your actual ID: `"ref":123`
   - Save the file (and if on Live, upload the modified file).
5. **Clear Editor Overrides:** In the target environment, go to **Appearance → Editor → Patterns**. Under **Template Parts**, click **Manage all template parts**. Hover over **`navigation`**, click the three dots (`⋮`), and hit **Reset** (or 'Clear customizations'). This forces WordPress to read your updated code file.
